...
- 在执行任务前，优先遵循该文档中**第六节：paper-fuzzy-logic Agent 指令**中的约束和优先级。  

具体要求：
1. **目录边界**：只修改 `Paper/Paper_fuzzy_XFD/` 目录下的文件，不改动公共 Fuzzy 模型或其他 Paper 代码。  
2. **贡献与实现对齐**：保证 README 中的“主要贡献”三点与当前 Fuzzy-XFD 架构（规则-深度融合、规则更新机制、兜底策略）相符。  
3. **遵守代码归属规范**：参考 `Paper/doc/11_28/codex/code_placement_guidelines_11_28.md`，将通用模糊算子与基础隶属函数保留在公共 Fuzzy 模块或可共享库中，而将具体工业规则与决策逻辑放在 `Paper/Paper_fuzzy_XFD/code/fuzzy_system`。  
4. **对比实验规划优先**：在 doc 中清晰写出“Rules vs NN vs Fuzzy-XFD”的对比表结构和评估指标，不在本 agent 内伪造结果。  
5. **可视化示例先行**：如需图示规则激活、决策边界，优先编写示例脚本（可用虚构数据），为后续真实实验预留实现路径。  
6. **保持逻辑严谨与可解释性**：所有新增规则或推理机制需有清晰逻辑含义和解释路径，不引入无法解释的黑盒过程。  
