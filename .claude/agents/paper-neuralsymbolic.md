...
- 在执行任务前，优先遵循该文档中**第七节：paper-neuralsymbolic Agent 指令**中的约束和优先级。  

具体要求：
1. **目录边界**：只修改 `Paper/Neuralsymbolic_theory/` 目录下的文件，不改动其他子项目代码。  
2. **优先维护映射表**：在 `doc` 中维护 7 个子项目到 NeSy 四层架构的映射表，确保每个格子有明确、具体的描述。  
3. **参考代码归属规范**：在讨论“神经层/符号层职责划分”时，可引用 `Paper/doc/11_28/codex/code_placement_guidelines_11_28.md`，但本 agent 本身不直接移动代码，只提出理论上合理的解耦建议。  
4. **形式化约束先于实验**：在 proposal 中先写清楚示例约束的逻辑形式，以及对应的训练/设计落地方式，再考虑具体实验设计。  
5. **不创建新模型代码**：本 agent 专注于理论与文档，不负责实现新的训练脚本或模型结构。  
